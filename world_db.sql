...
-- Дата скрипта: 02.03.2017 15:43:31
...
--
-- Описание для таблицы country_language
--
...
--
-- Описание для представления v_city
--
DROP VIEW IF EXISTS v_city CASCADE;
CREATE OR REPLACE 
	 
VIEW v_city
AS
	select `c`.`ID` AS `id`,`c`.`Name` AS `NameCity`,`c`.`CountryCode` AS `CountryCode`,`country`.`Code2` AS `Code2`,`c`.`District` AS `District`,`c`.`Population` AS `Population`,`country`.`Name` AS `NameCountry`,(case when (`cy2`.`Capital` is not null) then 'Capital' else NULL end) AS `Capital` from ((`city` `c` left join `country` on((`c`.`CountryCode` = `country`.`Code`))) left join `country` `cy2` on((`c`.`ID` = `cy2`.`Capital`))) order by `country`.`Name`;

--
-- Описание для представления v_country
--
DROP VIEW IF EXISTS v_country CASCADE;
CREATE OR REPLACE 
	 
VIEW v_country
AS
	select `c`.`Name` AS `Name`,`c`.`Population` AS `CountryPopulation`,`city`.`Name` AS `Capital`,`city`.`Population` AS `CapitalPopulation`,`c`.`Code` AS `Code`,`c`.`Code2` AS `Code2`,(select distinct concat(`cl`.`Language`,' (',max(`cl`.`Percentage`),'%',')') from `country_language` `cl` where ((`c`.`Code` = `cl`.`CountryCode`) and (`cl`.`IsOfficial` = 'T')) group by `cl`.`CountryCode`) AS `Lng`,`c`.`Region` AS `Region`,`c`.`SurfaceArea` AS `SurfaceArea`,`c`.`GovernmentForm` AS `GovForm` from (`country` `c` left join `city` on((`c`.`Capital` = `city`.`ID`)));
...
-- 
-- Вывод данных для таблицы country_language
--
